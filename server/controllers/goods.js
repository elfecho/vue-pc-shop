...
const OrdersDetailModel = require('../models/OrdersDetailModel.js');
...
exports.addOrder = async (ctx) => {
	...
	console.log('goodsList::::',orderObj.goodsList)
	try {
		const order = await OrderModel.create({
			userId: jwt.verify(orderObj.token, 'chambers'),
			addrId: orderObj.addrId,
			ordernum: utils.randomOrderNum(),
			remarks: orderObj.remarks,
			state: orderObj.state,
			updatetime: new Date(),
			createtime: new Date()
		});
		for(let item of orderObj.goodsList){
			const spec = await OrdersDetailModel.create({
				orderId:order.id,
...
exports.getCart = async (ctx) => {
	...
	orders = await CartModel.findAll({
		where: {
			userId
		},
		order: [
...
// 订单详情
// exports.addOrderDetail = async (ctx) => {
// 	const userId = jwt.verify(ctx.query.token, 'chambers');
